...
$getid = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM blog_categories where blog_cat_name = '$getthis'"));
$categoryid = $getid['blog_category_id'];
	...
	<div class="container-fluid no-padding pagebanner">
		<div class="container">
			<div class="pagebanner-content">
				<h3><?php echo(ucwords($getthis)); ?></h3>
				<ol class="breadcrumb">
					<li><a href="index.html">Home</a></li>
					<li><?php echo($getthis); ?></li>
				</ol>
			</div>
		</div>
	</div><!-- PageBanner /- -->
	<div class="container blog">
		...
		<div class="row">
			...
			<div class="col-md-9 col-sm-8 content-area">
							 <?php 
    $fetchallpostsforpagination = mysqli_num_rows(mysqli_query($conn,"SELECT * from blog_post where blog_post.topic_cat_id = '$categoryid'"));
    $numberofposts = 6;
    $pagination = ceil($fetchallpostsforpagination/$numberofposts);
        if ($page_num == '' || $page_num ==1) {
           $fetchallposts = mysqli_query($conn,"SELECT blog_post.*, users.* from blog_post INNER JOIN users WHERE blog_post.topic_by = users.profile_id AND blog_post.topic_cat_id = '$categoryid' ORDER BY topic_date DESC LIMIT 6 ");
          }else{
            $fetchallposts = mysqli_query($conn,"SELECT blog_post.*, users.* from blog_post INNER JOIN users WHERE blog_post.topic_by = users.profile_id AND blog_post.topic_cat_id = '$categoryid' ORDER BY topic_date DESC LIMIT 6 offset $next_page ");
          }
    ...
    <?php 
          ...
          ?>
            ...
            for ($i=0; $i < $pagination ; $i++) {
             if ($page_num == '' || $page_num ===1) {
               echo(' <li><a href="'.$getthis.'/'.($i+1).'" class="white-text">'.($i+1).'</a></li>');
              }else{
                echo(' <li><a href="../'.$getthis.'/'.($i+1).'" class="white-text">'.($i+1).'</a></li>');
              }
            }
