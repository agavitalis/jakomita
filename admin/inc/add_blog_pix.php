	...
	if (count($_FILES['attachment'])> 0) {
			for ($pixnum=0; $pixnum < count($_FILES['attachment']['name']) ; $pixnum++) { 
				...
				if (!isset($err_flag)) {
					$upload_date= time();
					$random = rand(0,10000);
					$customedName = $upload_date.$random.'.png';
					$file_dir = "../../uploads/".$customedName;
					$filenewpath = "../uploads/".$customedName;
					$send = move_uploaded_file($tmp_name, $file_dir);
					if ($send) {
						$insert_attachment = mysqli_query($conn,"INSERT INTO blog_pictures (file_name,file_path) VALUES ('$customedName','$filenewpath')");
						if ($insert_attachment) {
							$findinputedpix = mysqli_query($conn,"SELECT * FROM blog_pictures where file_name = '$customedName'");
							$row = mysqli_fetch_array($findinputedpix);
							$id= $row['blog_attachment_id'];
